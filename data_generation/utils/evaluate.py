...
def create_confusion_matrix_analysis(df):
    ...
    tp = len(
        df[(df["original_label"] == "safe") & (df["success"] == True)]
    )  # True Positive
    tn = len(
        df[(df["original_label"] == "unsafe") & (df["success"] == True)]
    )  # True Negative
    fn = len(
        df[(df["original_label"] == "safe") & (df["success"] == False)]
    )  # False Negative
    fp = len(
        df[(df["original_label"] == "unsafe") & (df["success"] == False)]
    )  # False Positive
    ...
    print(
        f"  Precision:   {precision:.4f} ({precision:.2%}) - Of predicted 'safe', how many were actually safe"
    )
    print(
        f"  Recall:      {recall:.4f} ({recall:.2%}) - Of actual 'safe', how many were correctly identified"
    )
    print(
        f"  Specificity: {specificity:.4f} ({specificity:.2%}) - Of actual 'unsafe', how many were correctly identified"
    )
    ...
    y_true = df["original_label"].map(
        {"safe": 1, "unsafe": 0}
    )  # safe=1 (positive), unsafe=0 (negative)
