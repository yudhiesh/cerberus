...
def init_argilla() -> rg.Argilla:
    ...
    return rg.Argilla(api_url="http://localhost:6900", api_key="argilla.apikey")
...
def upload_to_argilla(
    ...
    dataset_name: str = typer.Option(
        "misclassified-prompts", help="Name of the dataset in Argilla."
    ),
):
    ...
    try:
        ...
        dataset.records.log(
            records=records, mapping={"input_prompt": "prompt", "label": "label"}
        )

        typer.echo(
            f"Successfully uploaded {len(df)} records to Argilla dataset '{dataset_name}'"
        )
        typer.echo(
            "Please annotate the data in the Argilla UI at http://localhost:6900"
        )
...
def download_from_argilla(
    ...
    output_dir: Path = typer.Option(
        "misclassified-prompts_annotated",
