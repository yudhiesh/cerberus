...
def generate_dataset(
    argilla_export_dir: Path = typer.Option(
        ..., help="Path to Argilla export directory (with records.json)."
    ),
    judged_jsonl: Path = typer.Option(..., help="Path to judged dataset JSONL file."),
    output_file: Path = typer.Option(
        "full_dataset.jsonl", help="Path to save the merged and deduplicated dataset."
    ),
    ...
    argilla_df = pd.DataFrame(
        {
            ...
            "label": df["responses"].apply(
                lambda x: next(
                    (resp["value"] for resp in x.get("label", []) if "value" in resp),
                    None,
                )
            ),
        }
    ...
    result_df = merged_df[["input_prompt", "final_label"]].rename(
    ...
    if duplicates_count > 0:
        ...
        if len(conflicting_prompts) > 0:
            ...
            for prompt in conflicting_prompts.head(3).index:
                conflict_labels = result_df[result_df["input_prompt"] == prompt][
                    "label"
                ].unique()
            ...
            priority_df = priority_df.sort_values(
                ["input_prompt", "has_argilla"], ascending=[True, False]
            )
            priority_df = priority_df.drop_duplicates(
                subset=["input_prompt"], keep="first"
            )
            priority_df["final_label"] = priority_df["label"].fillna(
                priority_df["original_label"]
            )
            result_df = priority_df[["input_prompt", "final_label"]].rename(
                columns={"final_label": "label"}
            )
    ...
    typer.echo(
        f"- Coverage: {argilla_used / len(result_df) * 100:.1f}% argilla annotations"
    )
...
def upload_to_huggingface(
    input_file: Path = typer.Option(
        ..., help="Path to the processed JSONL file (output of generate-dataset)."
    ),
    repo_id: str = typer.Option(
        "yudhiesh/cerberus-guardrails-small",
        help="HuggingFace repo name, e.g. 'yudhiesh/cerberus-guardrails-small'.",
    ),
    test_size: float = typer.Option(
        0.2, help="Proportion of dataset to include in the test split."
    ),
    api_url: str = typer.Option("http://localhost:6900", help="Argilla API URL."),
    api_key: str = typer.Option("argilla.apikey", help="Argilla API key."),
):
